// Code adapted from: https://github.com/open-lambda/open-lambda/blob/3fe5547e/src/worker/event/lambdaServer.go
package event
...
func (s *LambdaServer) RunLambda(w http.ResponseWriter, r *http.Request) {
	...
	/*******************************************************/
	// log turnaraound time
	query := r.URL.Query()
	requestIndex := query.Get("id")
	if requestIndex != "" {
		defer func() {
			log.Printf("*** Request %s %d %v\n", requestIndex, t.Milliseconds, t.StartTime)
		}()
	}
	/*******************************************************/
	defer t.T1()
